...
class Field(object):
    ...
    def __init__(self, label=None, validators=None, filters=tuple(),
        ...
        self.label = Label(self.id, label if label is not None else self.gettext(
...
class SelectFieldBase(Field):
    ...
    def __iter__(self):
        opts = dict(widget=self.option_widget, _name=self.name,
                    _form=None, _meta=self.meta)
    ...
    def pre_validate(self, form):
        if self.data:
            ...
            for d in self.data:
                if d not in values:
                    raise ValueError(self.gettext(
                        "'%(value)s' is not a valid choice for this field") % dict(value=d))
...
class LocaleAwareNumberField(Field):
    ...
    def __init__(self, label=None, validators=None, use_locale=False, number_format=None, **kwargs):
        super(LocaleAwareNumberField, self).__init__(
    ...
    def process(self, formdata, data=unset_value):
        ...
        if isinstance(data, dict):
            self.form = self.form_class(
                formdata=formdata, prefix=prefix, **data)
        else:
            self.form = self.form_class(
